...
- Plugin bridge endpoint: `src/core/application/runtime/runtime.utils.ts` (oRPC `/rpc`, session->conversation forwarding)

## Adding a new bridge

1. Create `src/bridges/<bridge>/<bridge>.definition.ts`.
2. Create `src/bridges/<bridge>/<bridge>.bridge.ts` adapter.
3. Register definition in `src/core/application/bridge-registry/bridge-registry.utils.ts`.
4. Add config schema entries in `src/infra/config/config/config.schema.ts`.
