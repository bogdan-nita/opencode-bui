...
import { discoverConfigContext, readRuntimeConfig } from "@core/config";
import { ensureDir, fileExists, writeTextFile } from "@infra/runtime/runtime-fs";
import { allBridgeDefinitions } from "@core/application/bridge-registry";
...
export async function runOnboarding(): Promise<OnboardResult> {
  ...
  const pluginFileTemplate = [
    ...
    "",
    "export const BridgePlugin = OpenCodeBuiPlugin;",
    "",
  ].join("\n");
...
export type { OnboardResult } from "./onboard.types";
