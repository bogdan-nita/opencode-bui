...
} from "@core/application/bridge-registry";
import { startRuntime } from "@core/application/runtime";
import { testBridgeConnectivity } from "@core/application/bridge-test";
import { runOnboarding } from "@bin/onboard";
import { ensureDir, fileExists } from "@infra/runtime/runtime-fs";
...
async function startRuntimeFromConfig(
  ...
  try {
    await startRuntime({ config: cfg, bridges });
  } finally {
    ...
    await withCliErrors(async () => {
      ...
      const lines = [
        ...
        ...(pluginFilePath
          ...
          : ["- Plugin file: skipped"]),
        "- Plugin tool: use bui_send from OpenCode (bridge auto-boots)",
