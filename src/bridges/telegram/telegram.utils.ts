...
import { splitLongText } from "@core/domain/bridge.utils";
import type { InboundEnvelope, OutboundEnvelope } from "@core/domain/envelope.types";
  ...
  if (envelope.buttons && envelope.buttons.length > 0) {
    logger.info(
      {
        ...
        buttonPayloads: envelope.buttons.map((row) => row.map((button) => encodeBridgeButtonPayload(button.id, button.value))),
      },
    ...
    for (const row of envelope.buttons) {
      row.forEach((button, idx) => {
        keyboard.text(button.label, encodeBridgeButtonPayload(button.id, button.value));
        if (idx < row.length - 1) {
          return;
        }
      });
