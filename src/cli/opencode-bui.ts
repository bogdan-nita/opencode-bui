...
async function startRuntimeFromConfig(
  ...
  logger.info(
    { lockPath: cfg.paths.lockPath },
    "[bui] Attempting singleton lock acquisition.",
  );
  const lockHandle = await lockService.acquire(cfg.paths.lockPath);
  logger.info(
    { lockPath: cfg.paths.lockPath },
    "[bui] Singleton lock acquired.",
  );
...
cli
  ...
  .action(async () => {
    await withCliErrors(async () => {
      const { targetRoot, configPath, envPath, pluginEnvPath, pluginFilePath } =
        await runOnboarding();
