...
import { splitCommand } from "../../domain/bridge.utils";
...
export function routeInbound(
  ...
  return Effect.gen(function* () {
    ...
    if (slash?.command === "cd") {
      ...
      if (!sessionId) {
        const created = yield* Effect.promise(() => openCodeClient.createSession({ conversationKey: key }));
...
export type { CommandRouterInput, CommandRouterOutput } from "./command-router.types";
