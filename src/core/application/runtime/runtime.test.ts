...
import type { BridgeAdapter, BridgeRuntimeHandlers } from "../../ports/bridge-adapter.types";
...
describe("bui runtime", () => {
  ...
  it("wires inbound handler and sends outbound", async () => {
    ...
    const createClientMock = vi.spyOn(await import("@infra/opencode/open-code-client"), "createOpenCodeClient").mockReturnValue({
      async createSession() {
    ...
    await startRuntime({ config, bridges: [bridge], waitForShutdown: false });
