...
import { createLibsqlAgentStore } from "./agent-store";
import { createFileMediaStore } from "./media-store";
import { createLibsqlPermissionStore } from "./permission-store";
import { createLibsqlSessionStore } from "./session-store";
...
describe("db utils", () => {
  ...
  afterEach(async () => {
    vi.resetModules();
    vi.doUnmock("@runtime/runtime-fs/runtime-fs.js");
    await Promise.all(tempDirs.splice(0).map(async (dir) => rm(dir, { recursive: true, force: true })));
  });

  it("finds migrations folder from first matching candidate", async () => {
    vi.doMock("@runtime/runtime-fs/runtime-fs.js", () => ({
      fileExists: async (path: string) => path === resolve(process.cwd(), "drizzle"),
    }));

    const { resolveMigrationsFolder: resolveMigrations } = await import("./db");
